    ...
    return (
        <div>
            <div>
                {roomInfo && (
                    <div>
                        <h1>Players:</h1>
                        <ul>
                            {roomInfo.players.map((user, index) => (
            ...
            {gameOver ? (
                <div>
                    <h1>Game Over</h1>
                    <h2>{winner === color ? "You Win!" : "You Lose"}</h2>
                    <h3>Rematch Votes: {numVotes}</h3>
                    <h3>
                        {votedRematch
                            ? "Waiting for opponent to vote for rematch"
                            : <button onClick={rematch}>Vote for Rematch</button>}
                    </h3>
                </div>
            ) : (
